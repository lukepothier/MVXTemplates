    ...
    public class Setup : MvxAndroidAppCompatSetup
{
    ...
    // CreateApp() is the only method for which an override is required in Setup
    ...
    // If it's not necessary for a service to be available so early in the process, consider initializing it during InitializeLastChance instead
    ...
    {
        var customPresenter = new MyPresenter(AndroidViewAssemblies);
